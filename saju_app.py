...
2. **일상어가 주인공, 전문용어는 참고**: 이 글을 읽는 사람은 사주를 공부하는 사람이 아니라 **자기 인생이 궁금한 일반 고객**입니다. 문장을 "전문용어 → 설명" 순서로 쓰지 말고, 반드시 "**일상어로 된 결과·의미** 먼저 → 전문용어는 괄호로 참고만" 순서로 쓰십시오.
   - ❌ "토금상관격의 수기다" (용어만 던짐)
   - ⭕ "재주가 뛰어나 어디서든 인정받는 사주다(토금상관격土金傷官格)" (일상어가 문장의 주인공, 용어는 괄호 참고)
   - 한 항목에 전문용어는 1~2개면 충분합니다. 그 외엔 전부 일상 언어로 쓰십시오.
