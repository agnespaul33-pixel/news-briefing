...
def render_sinsal_badges(fp: dict):
    ...
    if not badges:
        ...
        return
    badge_html = " ".join(
        f'<span style="display:inline-block;background:#fff3e0;color:#000000;border:1px solid #ffb74d;'
...
【 서술 원칙 — 반드시 지킬 것 】
1. **사실 기반, 해석은 깊이 있게**: 모든 문장은 위 【 SAZU 계산 결과 】에 실제로 나온 값(신살명·격국명·십성명·형충관계·오행 등)을 최소 하나 이상 직접 언급하며 전개하십시오. 다만 "근거 기반"이 곧 "판단 기준 자료에 있는 것만 말하기"는 아닙니다 — 그 사실을 설명하는 논리·통찰은 당신의 사주첩경·자평진전 지식을 마음껏 동원해 구체적이고 인상 깊게 풀어내십시오. 막연하고 상투적인 일반론(예: "성격이 유연합니다", "노력이 필요합니다"류의 어디에나 붙일 수 있는 말)만 쓰지 마십시오.
2. **일상어가 주인공, 전문용어는 참고 — 예외 없이 매번 지킬 것**: 이 글을 읽는 사람은 사주를 공부하는 사람이 아니라 **자기 인생이 궁금한 일반 고객**입니다. 위 [판단 기준 자료]는 지니님이 공부용으로 정리한 원문 그대로라 전문용어·한자가 많은데, 그걸 그대로 옮기지 말고 **반드시 현대어로 먼저 풀어서 전달**하십시오.
   - 문장은 항상 "**일상어로 된 결과·의미** 먼저 → 전문용어(한자 포함)는 맨 뒤 괄호에만" 순서로 쓰십시오. 전문용어로 문장을 시작하거나 끝맺지 마십시오.
   - ❌ "토금상관격의 수기다" (용어만 던짐, 시작부터 한자어)
   - ❌ "재주가 뛰어난 토금상관격(土金傷官格)이라 어디서든 인정받는다" (용어가 문장 중간에 끼어 흐름을 끊음)
   - ⭕ "재주가 뛰어나 어디서든 인정받는 사주다(토금상관격土金傷官格)" (일상어가 문장의 주인공, 용어는 맨 뒤 괄호)
   - 한 항목에 전문용어는 1~2개면 충분합니다. 그 외엔 전부 일상 언어로 쓰십시오. 이 원칙을 지키지 않으면 유료 고객이 이해하지 못해 상담 가치가 떨어집니다 — 예외 없이 매 문장에 적용하십시오.
3. **짧고 압축적으로**: 각 항목은 원칙적으로 2~4문장을 넘기지 마십시오 — 근거가 많아도 그중 **가장 특징적인 것 1~2개만** 골라 압축해서 쓰고, 나머지는 과감히 생략하십시오.
4. **절대 반복 금지**: 같은 사실(예: 특정 신살·십성 강함)을 이미 한 항목에서 언급했으면, 다른 항목에서 또 언급하지 마십시오. 각 항목은 서로 다른 새로운 근거만 다루십시오.
...
7. "특이사항 없음"은 【 SAZU 계산 결과 】 자체에 관련 사실이 정말 없을 때만 쓰십시오(예: 자녀성에 해당하는 십성이 원국에 전혀 없음). 사실은 있는데 [판단 기준 자료]만 없는 경우는 "특이사항 없음"으로 얼버무리지 말고 원칙 1에 따라 당신의 지식으로 풀이하십시오.

다음 구조로, 아래 마커를 정확히 그대로 사용해 작성하십시오 (화면에서 자동으로 요약/펼치기 항목으로 나눠 보여주는 데 필요합니다):

## 핵심요약
이 사주에서 가장 특징적인 것 2~4가지만 골라 3~5문장으로 압축하십시오. 일상어로 결과부터 말하고, 전문용어는 괄호로만(위 원칙 2 참고). 격국명·용신·현재 대운·눈에 띄는 신살 중 임팩트 있는 것 위주로.
...
## 재물운
재성(정재·편재)의 유무·강약과 대운·세운의 재물 흐름을 짧게. 위 [대운표]에서 **현재 나이부터 70세까지**의 대운만 살펴 재성이 뚜렷하게 들어오거나 강해지는 시기를 찾아 "몇 세부터(어떤 대운) 재물운이 특히 좋아진다"처럼 구체적으로 짚어주십시오. 70세를 넘는 대운은 절대 언급하지 마십시오. 70세 이전에 뚜렷한 시기가 없으면 억지로 만들지 말고 이 언급은 생략하고 현재 흐름만 짧게 쓰십시오.

## 직업·적성
[직업유형 참고표]에서 이 사주의 십성 조합과 가장 가깝게 일치하는 직업 1~3개만 골라 짧게 제시. 참고표에 정확히 맞는 게 없으면 십성 특성 기반으로 일반적 적성만.

## 대인관계
비겁·도화 등 대인관계 관련 신살·십성을 짧게. {gender_label}성 배우자성({spouse_star})도 이 항목에서 함께. 위 [대운표]에서 **현재 나이부터 70세까지**의 대운만 살펴 배우자성({spouse_star})이나 도화가 뚜렷하게 들어오거나 강해지는 시기를 찾아 "몇 세부터(어떤 대운) 애정운/이성운이 특히 좋아진다"처럼 구체적으로 짚어주십시오. 70세를 넘는 대운은 절대 언급하지 마십시오. 70세 이전에 뚜렷한 시기가 없으면 억지로 만들지 말고 이 언급은 생략하고 현재 흐름만 짧게 쓰십시오.
